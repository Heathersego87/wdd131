...
console.log(searchList(simpleList, "b")); // should show ["Bananas", "broccoli"]


// ===============================
// ACTIVITY 3: Search hikes (name + description + tags + sorted)
// ===============================
function searchHikes(list, query) {
  ...
  function searchCallback(hike) {
    ...
    const inDescription = hike.description.toLowerCase().includes(q);
  ...
  // filter hikes
  const results = list.filter(searchCallback);

  // sort the filtered hikes by distance (number)
  results.sort((a, b) => {
